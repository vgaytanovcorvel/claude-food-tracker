...
describe('BookmarksPage', () => {
  it('render_ShouldShowTitle_WhenRendered', () => {
    ...
    // Assert
    expect(screen.getByText('Saved Alternatives')).toBeInTheDocument()
  })

  it('render_ShouldShowBookmark_WhenDataLoaded', async () => {
    // Arrange
    localStorage.setItem('misteryapp:userId', '1')
    const services = makeServices()
    services.bookmarkService.getBookmarks = vi.fn().mockResolvedValue([{
      id: 1,
      userId: 1,
      alternativeFoodName: 'Salad',
      imageBase64: null,
  ...
  it('render_ShouldShowError_WhenFetchFails', async () => {
    ...
    localStorage.setItem('misteryapp:userId', '1')
    const services = makeServices()
    services.bookmarkService.getBookmarks = vi.fn().mockRejectedValue(new Error('Network error'))
