...
describe('useDeleteBookmark', () => {
  it('useDeleteBookmark_ShouldCallService_WhenMutated', async () => {
    // Arrange
    const services = makeServices()
    vi.mocked(services.bookmarkService.deleteBookmark).mockResolvedValue(undefined as never)
    const wrapper = makeWrapper(services)

    // Act
    const { result } = renderHook(() => useDeleteBookmark('1'), { wrapper })
    await act(async () => {
      await result.current.mutateAsync(1)
    })

    // Assert
    expect(services.bookmarkService.deleteBookmark).toHaveBeenCalledWith(1)
  })
})
