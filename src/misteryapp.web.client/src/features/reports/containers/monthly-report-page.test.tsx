...
import { MonthlyReportPage } from '../containers/monthly-report-page'
      ...
      <QueryClientProvider client={queryClient}>
        <ServicesProvider services={services}>
          <Routes>
            ...
            <Route path="/reports/weekly" element={<div>Weekly Report Page</div>} />
          </Routes>
        </ServicesProvider>
      </QueryClientProvider>
    </MemoryRouter>
...
describe('MonthlyReportPage', () => {
  ...
  it('render_ShouldShowMotivatingCopy_WhenReportLoaded', async () => {
    ...
    localStorage.setItem('misteryapp:userId', '1')
    const services = makeServices()
    services.reportService.getMonthlyReport = vi.fn().mockResolvedValue({
      monthStart: '2024-01-01',
      monthEnd: '2024-01-31',
      dailySummaries: [],
      totalCalories: 7500,
    ...
    // Assert
    await waitFor(() => expect(screen.getByText('Great month!')).toBeInTheDocument())
  })
  ...
  it('render_ShouldShowError_WhenFetchFails', async () => {
    ...
    localStorage.setItem('misteryapp:userId', '1')
    const services = makeServices()
    services.reportService.getMonthlyReport = vi.fn().mockRejectedValue(new Error('Network error'))
  ...
  it('render_ShouldShowTotalCalories_WhenReportLoaded', async () => {
    ...
    // Assert
    await waitFor(() => expect(screen.getByText('7500 kcal total')).toBeInTheDocument())
  })
})
