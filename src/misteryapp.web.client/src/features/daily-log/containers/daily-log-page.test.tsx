...
import { DailyLogPage } from '../containers/daily-log-page'
...
describe('DailyLogPage', () => {
  ...
  it('render_ShouldShowEntry_WhenDataLoaded', async () => {
    // Arrange
    localStorage.setItem('misteryapp:userId', '1')
    const services = makeServices()
    services.foodLogService.getDailyEntries = vi.fn().mockResolvedValue([{
      id: 10,
      userId: 1,
      foodName: 'Grilled Chicken',
      estimatedCalories: 350,
  ...
  it('render_ShouldShowError_WhenFetchFails', async () => {
    ...
    localStorage.setItem('misteryapp:userId', '1')
    const services = makeServices()
    services.foodLogService.getDailyEntries = vi.fn().mockRejectedValue(new Error('Network error'))
  ...
  it('render_ShouldShowPrevNextButtons_WhenRendered', () => {
    // Arrange
    localStorage.setItem('misteryapp:userId', '1')
    const services = makeServices()
