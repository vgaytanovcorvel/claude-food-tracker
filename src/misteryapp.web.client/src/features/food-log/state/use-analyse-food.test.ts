...
import { useAnalyseFood } from './use-analyse-food'
...
describe('useAnalyseFood', () => {
  it('useAnalyseFood_ShouldCallService_WhenMutated', async () => {
    // Arrange
    ...
    vi.mocked(services.foodLogService.analyseEntry).mockResolvedValue(mockAnalysisResult as never)
    const wrapper = makeWrapper(services)

    // Act
    const { result } = renderHook(() => useAnalyseFood(), { wrapper })
    await act(async () => {
      await result.current.mutateAsync({ entryId: 10 })
    })

    // Assert
    expect(services.foodLogService.analyseEntry).toHaveBeenCalledWith(10, undefined)
  })
})
